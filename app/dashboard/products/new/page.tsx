...
export default function NewProductPage() {
  ...
  const checkProductLimit = async () => {
    ...
    if (store) {
      ...
      if (plan && plan.max_products !== -1 && (count || 0) >= plan.max_products) {
        alert(`Você atingiu o limite de ${plan.max_products} produtos do plano gratuito. Faça upgrade para adicionar mais produtos.`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (plan && plan.max_products !== -1 && (count || 0) >= plan.max_products) {
      alert(`Limite de produtos atingido! Você pode cadastrar até ${plan.max_products} produtos no plano gratuito.`)
      setLoading(false)
      return
    }
    ...
    if (error) {
      ...
      // Mark tutorial as completed if it was the first product
      if (showTutorial) {
        await supabase
          .from('stores')
          .update({ first_product_tutorial_completed: true })
          .eq('id', store.id)
      }
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2 font-avenir">Novo Produto</h1>
          {productCount > 0 && (
            <p className="text-sm text-gray-600">
              Você tem {productCount} produto(s) cadastrado(s). 
              {productCount < 5 && (
                <span className="text-[#004DF0]"> Restam {5 - productCount} no plano gratuito.</span>
              )}
            </p>
          )}
        </div>
